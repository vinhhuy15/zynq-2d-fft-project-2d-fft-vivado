...
typedef struct {
    ...
    XFft2d_fixed_top_uio_map maps[ MAX_UIO_MAPS ];
} XFft2d_fixed_top_uio_info;

/***************** Variable Definitions **************************************/
static XFft2d_fixed_top_uio_info uio_info;
...
static int uio_info_read_name(XFft2d_fixed_top_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XFft2d_fixed_top_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XFft2d_fixed_top_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XFft2d_fixed_top_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XFft2d_fixed_top_Initialize(XFft2d_fixed_top *InstancePtr, const char* InstanceName) {
	XFft2d_fixed_top_uio_info *InfoPtr = &uio_info;
